...
func init() {
	...
	case "darwin", "linux":
		migrationsDir = config.AirQoIntegratorConf.Server.MigrationsDirectory
	default:
	...
	m, err := migrate.New(
		...
		migrationsDir,
		config.AirQoIntegratorConf.Database.URI)
	if err != nil {
		log.Fatal(err)
	}
...
func CreateBaseDHIS2Server() {
	...
	metadataServer.s.URL = config.AirQoIntegratorConf.API.AIRQODHIS2BaseURL + "/metadata.json"
	metadataServer.s.Username = config.AirQoIntegratorConf.API.AIRQODHIS2User
	metadataServer.s.Password = config.AirQoIntegratorConf.API.AIRQODHIS2Password
	metadataServer.s.AuthMethod = "Basic"
	metadataServer.s.AuthToken = config.AirQoIntegratorConf.API.AIRQODHIS2PAT
...
"importReportMode": "DEBUG"}`), &urlParams); err != nil {
	...
	ouGroupAddServer.s.URL = config.AirQoIntegratorConf.API.AIRQODHIS2BaseURL + "/organisationUnitGroups"
	...
	ouUpdateServer.s.URL = config.AirQoIntegratorConf.API.AIRQODHIS2BaseURL + "/organisationUnits"
...
func SyncLocationsToServer(serverName string) {
	...
	p.Add("paging", "true")
	p.Add("pageSize", "1")
	p.Add("level", fmt.Sprintf("%d", config.AirQoIntegratorConf.API.AIRQODHIS2FacilityLevel))
	...
	if respBody != nil {
		...
		for level := 1; level < config.AirQoIntegratorConf.API.AIRQODHIS2FacilityLevel; level++ {
			...
			//	}
			//}
			ouChunks := lo.Chunk(syncOus, config.AirQoIntegratorConf.API.AIRQOMetadataBatchSize)
			...
			for _, chunck := range ouChunks {
				...
				if level < config.AirQoIntegratorConf.API.AIRQODHIS2FacilityLevel-1 {
