...
func GetUID() string {
	...
	s.Grow(11) // Pre-allocate memory to improve performance

	// Ensure the first character is an uppercase letter from the alphabet
	s.WriteByte(allowedCharacters[11+rand.Intn(len(allowedCharacters)-11)]) // Convert to uppercase
...
//
//	// Add basic authentication
//	auth := config.AirQoIntegratorConf.API.AIRQO + ":" + config.MFLIntegratorConf.API.MFLPassword
//	basicAuth := "Basic " + base64.StdEncoding.EncodeToString([]byte(auth))
//	req.Header.Set("Authorization", basicAuth)
//
//	// Create custom transport with TLS settings
